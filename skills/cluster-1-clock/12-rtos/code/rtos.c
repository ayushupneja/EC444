...
static void test_alpha_display() {
    ...
    while (1) {
      if (direction == 0) {
        displaybuffer[0] = alphafonttable[(int)'U'];   // T.
        displaybuffer[1] = alphafonttable[(int)'P'];  // D.
        displaybuffer[2] = alphafonttable[(int)'-'];  // C.
        displaybuffer[3] = alphafonttable[(int)'-'];  // L.
      }
      else {
        displaybuffer[0] = alphafonttable[(int)'D'] ;  // T.
        displaybuffer[1] = alphafonttable[(int)'O'];  // D.
        displaybuffer[2] = alphafonttable[(int)'W'];  // C.
        displaybuffer[3] = alphafonttable[(int)'N'];  // L.
      }
...
static void check_button() {
  ...
  while(1) {
    ...
    int button_pressed;
      ...
      button_pressed = gpio_get_level(A1);
      //printf("Button val: %d\n", button_pressed);
...
//    free(pointer);
    ...
    xTaskCreate(check_button, "check_button", 4096, NULL, configMAX_PRIORITIES-5, NULL);
    ...
    while(1){
      for (i=0;i<=15;i++){
        if (direction==0) {
          ...
          binaryNum = decimal_to_binary(15-i);
        }
